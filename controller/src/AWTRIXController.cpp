...
uint32_t hostDisconnectedAt = 0;
uint32_t lastHostReconnectAttempt = 0;
const uint32_t HOST_RECONNECT_INTERVAL_MS = 5000;
const uint32_t HOST_RESTART_TIMEOUT_MS = 90000;
...
void drawOfflineClock()
{
	...
	struct tm localTime;
	if (now <= 1700000000 || localtime_r(&now, &localTime) == nullptr)
	{
		return;
	}
...
void setup()
{
	...
	WiFi.mode(WIFI_STA);
	// Keep local time available while the remote Host is offline (China Standard Time).
...
void loop()
{
	...
	if (firstStart && !ignoreServer)
	{
		...
		{
			if (!client.connected() && hasValidLocalTime())
			{
				drawOfflineClock();
			}
			else
	...
	if (!updating)
	{
		...
		if (WIFIConnection || firstStart)
		{
			...
			{
				uint32_t now = millis();
				if (hostDisconnectedAt == 0)
				{
					hostDisconnectedAt = now;
				}

				// Throttle reconnects so a remote Host outage cannot lock the ESP8266
				// in a tight TCP connect loop.
				if (lastHostReconnectAttempt == 0 || now - lastHostReconnectAttempt >= HOST_RECONNECT_INTERVAL_MS)
				{
					lastHostReconnectAttempt = now;
					reconnect();
				}

				// If the network stack cannot recover for 90 seconds, reboot locally.
				// WiFiManager settings live in flash, so Wi-Fi and Host configuration remain intact.
				if (!client.connected() && now - hostDisconnectedAt >= HOST_RESTART_TIMEOUT_MS)
				{
					ESP.restart();
				}
				if (WIFIConnection)
